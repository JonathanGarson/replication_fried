/*
Program: nelder.c
Purpose: Shell program for numerical recipies nelder-meade routine
Calls: ucont.c amoeba.c amoeba_r.c 
*/
...

{
	...
	if (own ==1){
		hpprime1 = prime1[0]; haprime1 = prime1[1]; aprime1 = prime1[2]; xprime1 = prime1[3];  
		hpprime2 = prime2[0]; haprime2 = prime2[1]; aprime2 = prime2[2]; xprime2 = prime2[3]; 
	}else {
	...
	if(own==1){
		for (counter =0; counter <1; counter++){
			...
			amoeba(p, y, ndim, ftol, ucont, &nfunc, m , hs, zi, rho, Omega, w, ar, ph, ah, own, qi,  na, pr_amoeba); 
			...
			if (counter < 3){
				...
				xprime2 = 0.5*Omega*exp(-theta*pow(haprime2/hpprime2, 1.0/theta))*hpprime2; 
			}
