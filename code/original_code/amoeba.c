/*Modified from Numerical Recipies in C (Press et al. 1988)*/
	...
	*nfunk =0;
	...
	GET_PSUM
		for (;;) {
			...
			*nfunk+=2; 
			ytry = amotry(p, y, psum, ndim, funk, ihi, -1.0, m, hs, zi, rho, Omega, w, ar, ph, ah, own,qi, na, pr);

			if(ytry <=y[ilo])
				ytry = amotry(p, y, psum, ndim, funk, ihi, 2.0, m, hs, zi, rho, Omega, w, ar, ph, ah, own, qi, na,  pr); 
			else if (ytry >= y[inhi]){
				ysave = y[ihi];
				ytry = amotry(p, y, psum, ndim, funk, ihi, 0.5, m, hs, zi,rho, Omega, w, ar, ph, ah, own,qi, na, pr); 
				if (ytry >=ysave) {
					for ( i =0; i< mpts; i++){
						if (i != ilo) {
							for (j = 0; j<ndim; j++){
							...
							y[i] = (*funk)(psum, m, hs, ah,  zi, rho, Omega,  w, ar, ph, own, qi, na,  pr);
						}
					}
					*nfunk +=ndim; 
					GET_PSUM
